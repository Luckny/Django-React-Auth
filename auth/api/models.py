...
class UserManager(BaseUserManager):
    # override create user function. A user is created with an email and a password
    # **extras: fields like first_name, last_name ...
    def create_user(self, email, password, **extras):
        if not email:
            raise ValueError('Users must have an email address')
        
        user = self.model(email = self.normalize_email(email), **extras) # unsaved user model with normalized email
